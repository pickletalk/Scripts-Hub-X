-- ================================
-- Scripts Hub X | Official (Fixed Cross-Client Detection)
-- ================================
...
-- ================================
-- SCRIPTS HUB X USER TRACKING SYSTEM (REPLICATED)
-- ================================
...
-- Create user marker in ReplicatedStorage (visible to all clients)
local function createUserMarker()
	pcall(function()
		local markerName = SCRIPTSHUBX_MARKER_NAME .. "_" .. player.UserId
		
		-- Check if marker already exists
		if ReplicatedStorage:FindFirstChild(markerName) then
			print("[SHX] Marker already exists, updating...")
			userMarker = ReplicatedStorage[markerName]
		else
		...
		local userInfo = userMarker:FindFirstChild("UserInfo")
		if not userInfo then
			userInfo = Instance.new("StringValue")
...
local function createCommandFolder()
	pcall(function()
		...
		print("[SHX] Command folder initialized in ReplicatedStorage")
	end)
end

-- Scan for other Scripts Hub X users in ReplicatedStorage
...
local selfOnlyCommands = {
	...
	"india", "scriptshubx", "anime", "gravity", "framerate", "uncontrol", "list"
}
...
spawn(function()
	...
	-- Initialize command system for all users (using ReplicatedStorage)
