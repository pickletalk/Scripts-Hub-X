...
MainSection:Button({
    Text = "Teleport to Highest Brainrot",
    Callback = function()
...
local leaveKeybind = "L"

UtilitiesSection:Keybind({
    Text = "Leave Keybind",
    Default = Enum.KeyCode.L,
    Callback = function(key)
        leaveKeybind = key.Name
    end
})
...
local placeIdLabel = ServerSection:Label({
    Text = "Place ID: " .. game.PlaceId
})

local jobIdLabel = ServerSection:Label({
    Text = "Job ID: " .. game.JobId
})

local playerCountLabel = ServerSection:Label({
    Text = "Players: " .. #Players:GetPlayers() .. " / " .. Players.MaxPlayers
})

local highestBrainrotLabel = ServerSection:Label({
    Text = "Highest Brainrot: Scanning..."
})
...
task.spawn(function()
    while true do
        ...
        if highest then
            highestBrainrotLabel:Set("Highest: " .. highest.price .. " | " .. highest.plotName .. " #" .. highest.podiumNumber)
        else
            highestBrainrotLabel:Set("Highest Brainrot: None found")
        end
    end
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    ...
    if input.KeyCode.Name == leaveKeybind then
        kickPlayer()
    end
end)
