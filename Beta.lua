-- ========================================
-- STEAL A ANIME - WINDUI HUB (OPTIMIZED)
...
-- Low GFX Storage (FIXED - stores original properties)
local LowGFXStorage = {
    OriginalProperties = {},
}
...
-- ========================================
-- SPEED SPOOFER (OPTIMIZED - NO FPS DROP)
-- ========================================
local SpeedSpoofer = {
    Enabled = false,
    SpoofedSpeed = 16,
...
local function setupSpeedSpoofer()
    ...
    -- Optimized __index with caching to prevent FPS drops
    mt.__index = newcclosure(function(self, key)
        -- Only intercept if speed is enabled and caller is from game (not our script)
        if States.SpeedEnabled and checkcaller() == false then
            if typeof(self) == "Instance" then
                -- Cache check to reduce overhead
                local cacheKey = tostring(self) .. key
                local currentTime = tick()
                
                -- Use cached result if recent (within 0.1 seconds)
                if SpeedSpoofer.Cache[cacheKey] and (currentTime - SpeedSpoofer.LastUpdate) < 0.1 then
                    if key == "WalkSpeed" and self:IsA("Humanoid") then
                        return SpeedSpoofer.SpoofedSpeed
                    end
                end
                
                -- Only check Humanoid WalkSpeed
                if key == "WalkSpeed" and self:IsA("Humanoid") then
                    SpeedSpoofer.Cache[cacheKey] = true
                    SpeedSpoofer.LastUpdate = currentTime
                    return SpeedSpoofer.SpoofedSpeed
                end
            end
        end
    ...
    -- Optimized __namecall with minimal overhead
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if States.SpeedEnabled and checkcaller() == false then
            if typeof(self) == "Instance" and self:IsA("Humanoid") then
                -- Block GetPropertyChangedSignal for WalkSpeed
                if method == "GetPropertyChangedSignal" and args[1] == "WalkSpeed" then
                    local dummySignal = Instance.new("BindableEvent").Event
                    return dummySignal
                end
            end
        end
    ...
    print("Speed Spoofer Initialized (Optimized - No FPS Drop)")
end

-- Initialize speed spoofer
task.spawn(function()
    setupSpeedSpoofer()
end)
...
local function toggleSpeed(state)
    ...
    if state then
        ...
        SpeedSpoofer.RealSpeed = States.SpeedValue
        humanoid.WalkSpeed = States.SpeedValue
        
        -- Optimized: Only update every 0.1 seconds instead of every frame
        Connections.Speed = RunService.Heartbeat:Connect(function()
            local h = getHumanoid()
            if States.SpeedEnabled and h then
                -- Throttle updates to prevent FPS drops
                if tick() - (Connections.SpeedLastUpdate or 0) > 0.1 then
                    h.WalkSpeed = States.SpeedValue
                    Connections.SpeedLastUpdate = tick()
                end
            end
        end)
    else
        if Connections.Speed then
            ...
            Connections.SpeedLastUpdate = nil
        end
...
-- ========================================
-- OPTIMIZATIONS TAB FEATURES (LOW GFX FIXED)
-- ========================================
...
-- LOW GFX WITH RESTORE FUNCTION (FIXED)
local function storeOriginalProperties(part)
    local partId = tostring(part)
    if not LowGFXStorage.OriginalProperties[partId] then
        LowGFXStorage.OriginalProperties[partId] = {
            Material = part:IsA("BasePart") and part.Material or nil,
            CastShadow = part:IsA("BasePart") and part.CastShadow or nil,
            Reflectance = part:IsA("BasePart") and part.Reflectance or nil,
            TextureID = part:IsA("MeshPart") and part.TextureID or nil,
            Enabled = (part:IsA("ParticleEmitter") or part:IsA("Trail") or part:IsA("Beam")) and part.Enabled or nil,
            Transparency = (part:IsA("Decal") or part:IsA("Texture")) and part.Transparency or nil,
        }
    end
end

local function applyLowGFXToPart(part)
...
local function restoreOriginalProperties(part)
    local partId = tostring(part)
    local stored = LowGFXStorage.OriginalProperties[partId]
    
    if stored then
        if part:IsA("BasePart") and stored.Material then
            part.Material = stored.Material
            part.CastShadow = stored.CastShadow
            part.Reflectance = stored.Reflectance
        end
        
        if part:IsA("MeshPart") and stored.TextureID then
            part.TextureID = stored.TextureID
        end
        
        if (part:IsA("ParticleEmitter") or part:IsA("Trail") or part:IsA("Beam")) and stored.Enabled ~= nil then
            part.Enabled = stored.Enabled
        end
        
        if (part:IsA("Decal") or part:IsA("Texture")) and stored.Transparency then
...
local function toggleLowGFX(state)
    ...
    if state then
        ...
        WindUI:Notify({
            ...
            Content = "All parts set to SmoothPlastic for performance!",
            Duration = 3,
            Icon = "zap",
        })
    else
        ...
        for _, obj in pairs(Workspace:GetDescendants()) do
            restoreOriginalProperties(obj)
        end
        
        -- Clear storage
        LowGFXStorage.OriginalProperties = {}
        ...
        WindUI:Notify({
            ...
            Content = "Graphics restored to original state!",
            Duration = 3,
            Icon = "check",
        })
    end
...
-- ========================================
-- CREDITS TAB ELEMENTS (GETCUSTOMASSET)
-- ========================================

-- Load custom image using getcustomasset
local thumbnailImage = "rbxassetid://86472170233220" -- Fallback
if getcustomasset then
    local success, customAsset = pcall(function()
        return getcustomasset("logo.png")
    end)
    if success and customAsset then
        thumbnailImage = customAsset
    end
end
