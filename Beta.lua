...
local States = {
    ...
    PlayerBaseTimeESPEnabled = false,
            ...
            if plotSign then
                ...
                if surfaceGui then
                    ...
                    if frame then
                        ...
                        if textLabel and textLabel.Text:find(LocalPlayer.Name .. "'s Base") then
                            return plot.Name
                        end
                    end
                end
...
-- ========================================
-- WALKSPEED SPOOF IMPLEMENTATION
-- ========================================
    ...
    Callback = function(state)
        ...
        if state then
            ...
            if not States.PlayerPlotName then
                ...
                States.InstantStealEnabled = false
                return
            end
            
            -- Hook __namecall to intercept FireServer
            if not States.OriginalNamecall then
                States.OriginalNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                    local args = {...}
                    local method = getnamecallmethod()
                    
                    if method == "FireServer" and self:IsA("RemoteEvent") then
                        if self.Name:match("^RE/") and #args >= 4 then
                            -- Check if it matches the steal pattern
                            if type(args[1]) == "number" and type(args[2]) == "string" and type(args[3]) == "string" and type(args[4]) == "number" then
                                if States.InstantStealEnabled and States.PlayerPlotName then
                                    task.spawn(function()
                                        local character = LocalPlayer.Character
                                        local rootPart = character and character:FindFirstChild("HumanoidRootPart")
                    ...
                    return States.OriginalNamecall(self, ...)
                end)
            end
...
local DesyncButton = MainTab:Button({
    ...
    Callback = function()
        if not LocalPlayer.PlayerGui:FindFirstChild("DesyncUI") then
            ...
            -- Toggle desync on click (not drag)
...
local SpeedToggle = PlayerTab:Toggle({
    Title = "Speed",
    Desc = "Adjust your walking speed (bypass anti-cheat)",
    Icon = "gauge",
    Default = false,
    Callback = function(state)
    ...
    Callback = function(state)
        ...
        if state then
            ...
            if plotsFolder then
                for _, plot in pairs(plotsFolder:GetChildren()) do
                    local plotSign = plot:FindFirstChild("PlotSign")
                    if plotSign and not plotSign:FindFirstChild("BaseESP") then
                        local highlight = Instance.new("Highlight")
            ...
            if plotsFolder then
                for _, plot in pairs(plotsFolder:GetChildren()) do
                    local plotSign = plot:FindFirstChild("PlotSign")
                    if plotSign then
                        local esp = plotSign:FindFirstChild("BaseESP")
...
-- Player Base Time ESP (ESP all plots with their timers)
local PlayerBaseTimeESPToggle = VisualTab:Toggle({
    Title = "Player Base Time ESP",
    Desc = "Shows remaining time on all plots",
    Icon = "clock",
    Default = false,
    Callback = function(state)
        States.PlayerBaseTimeESPEnabled = state
        ...
        if state then
                ...
                for _, plot in pairs(plotsFolder:GetChildren()) do
                    ...
                    if purchases then
                        ...
                        if plotBlock then
                            ...
                            if main and not main:FindFirstChild("TimeESP") then
                                local billboardGui = Instance.new("BillboardGui")
                                billboardGui.Name = "TimeESP"
                                ...
                                table.insert(States.BaseTimeESPObjects, billboardGui)
                            end
                        end
                    end
                end
            ...
            WindUI:Notify({
                Title = "Player Base Time ESP Enabled",
                Content = "Showing all plot timers",
                Duration = 3,
                Icon = "check",
            })
        else
            for _, espObject in pairs(States.BaseTimeESPObjects) do
                if espObject then
                    pcall(function() espObject:Destroy() end)
                end
            end
            States.BaseTimeESPObjects = {}
            
            WindUI:Notify({
                Title = "Player Base Time ESP Disabled",
...
-- Base Timer ESP (UI showing player's plot time)
local BaseTimerESPToggle = VisualTab:Toggle({
    Title = "Base Timer ESP",
    Desc = "Shows your base remaining time in UI",
    Icon = "timer",
    Default = false,
    Callback = function(state)
        ...
        if state then
            ...
            if not States.PlayerPlotName then
                ...
                States.BaseTimerESPEnabled = false
                return
            end
            ...
            if not LocalPlayer.PlayerGui:FindFirstChild("BaseTimerUI") then
                ...
                title.TextColor3 = Color3.fromRGB(255, 15, 123)
                title.TextSize = 13
...
myConfig:Register("PlayerBaseTimeESP", PlayerBaseTimeESPToggle)
...
WindUI:Popup({
    ...
    Icon = "sword",
    Content = "New Update: Added Instant Steal, Desync, Auto Lock, Player ESP, Base ESP, Speed Bypass, and more!",
    Buttons = {
        {
            Title = "Close",
...
-- Player Base Time ESP Update Loop (Updates all plot timers)
task.spawn(function()
    while task.wait(0.5) do
        if States.PlayerBaseTimeESPEnabled then
            local plotsFolder = Workspace:FindFirstChild("Plots")
            if plotsFolder then
                for _, plot in pairs(plotsFolder:GetChildren()) do
                    local purchases = plot:FindFirstChild("Purchases")
                    if purchases then
                        ...
                        if plotBlock then
                            ...
                            if main then
                                local billboard = main:FindFirstChild("BillboardGui")
                                local timeESP = main:FindFirstChild("TimeESP")
                                
                                if billboard and timeESP then
                                    local remainingTime = billboard:FindFirstChild("RemainingTime")
                                    local timeText = timeESP:FindFirstChild("TimeText")
                                    
                                    if remainingTime and timeText then
                                        timeText.Text = remainingTime.Text
                                        
                                        -- Color coding
                                        local timeValue = tonumber(remainingTime.Text:match("%d+"))
                                        if timeValue then
                                            if timeValue == 0 then
                                                timeText.TextColor3 = Color3.fromRGB(34, 197, 94) -- Green
                                            elseif timeValue <= 5 then
                                                timeText.TextColor3 = Color3.fromRGB(234, 179, 8) -- Yellow
                                            elseif timeValue <= 10 then
                                                timeText.TextColor3 = Color3.fromRGB(234, 179, 8) -- Yellow
                                            else
                                                timeText.TextColor3 = Color3.fromRGB(239, 68, 68) -- Red
                                            end
                                        end
                                    end
                                end
...
-- Base Timer ESP Update Loop (Updates player's plot timer UI)
task.spawn(function()
    while task.wait(0.5) do
...
if States.PlayerPlotName then
    WindUI:Notify({
        Title = "Plot Found",
        Content = "Your plot has been located: " .. States.PlayerPlotName,
        Duration = 4,
        Icon = "home",
    })
else
    WindUI:Notify({
        ...
        Content = "Some features may not work without a plot",
        Duration = 4,
        Icon = "alert-triangle",
    })
end
