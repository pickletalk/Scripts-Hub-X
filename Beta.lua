...
local function setupSpeedSpoofer()
    ...
    -- Hook __index to spoof WalkSpeed reading
    ...
    -- Hook __namecall for GetPropertyChangedSignal and other methods
        ...
        if States.SpeedEnabled and checkcaller() == false then
            if typeof(self) == "Instance" and self:IsA("Humanoid") then
                if method == "GetPropertyChangedSignal" and args[1] == "WalkSpeed" then
                    -- Return dummy signal that never fires
    ...
    print("🎯 Speed Spoofer Initialized - Server sees speed as 16!")
end
...
local function createStealUI()
    ...
    titleText.Text = "🗡️ ANIME HEIST 🗡️"
...
-- ========================================
-- PLAYER TAB FEATURES (SPEED FIXED)
-- ========================================
...
-- ========================================
-- OPTIMIZATIONS TAB FEATURES (LOW GFX ADDED)
-- ========================================
...
local SpeedToggle = PlayerTab:Toggle({
    Title = "Speed Changer (Anti-Detect)",
    Desc = "Custom walk speed with server spoofer - server sees 16!",
    Default = false,
    Callback = function(state)
        toggleSpeed(state)
    end
...
-- ========================================
-- OPTIMIZATIONS TAB ELEMENTS (LOW GFX ADDED)
-- ========================================
...
local LowGFXToggle = OptimizationsTab:Toggle({
    Title = "Low GFX Mode",
    Desc = "Convert all parts to SmoothPlastic for better performance",
    Default = false,
    Callback = function(state)
        toggleLowGFX(state)
    end
...
-- ========================================
-- CREDITS TAB ELEMENTS (THUMBNAIL FIXED)
-- ========================================
...
WindUI:Notify({
    Title = "Hub Loaded!",
    Content = "Steal A Anime Hub loaded with all fixes! Speed spoofer active.",
    Duration = 3,
    Icon = "check",
})
