...
-- Advanced Noclip Storage (SPEED-AWARE ANTI-CHEAT SYSTEM)
local NoclipData = {
    Character = nil,
    Humanoid = nil,
    HRP = nil,
    ...
    PositionHistory = {},
    HistorySize = 5,
    BaseAntiCheatThreshold = 7, -- Base threshold for normal speed (16)
    RayLength = 100,
    TeleportCooldown = 0,
    IsAutoLocking = false,
...
local AntiVoidData = {
    VoidThreshold = -50, -- If Y goes below this, teleport up
    LastValidPosition = nil,
}
...
-- ========================================
-- SPEED-AWARE ANTI-CHEAT NOCLIP SYSTEM (FIXED)
-- ========================================

-- Calculate dynamic threshold based on current speed
local function getSpeedAwareThreshold()
    local currentSpeed = States.SpeedValue
    local speedMultiplier = currentSpeed / 16 -- Ratio compared to normal speed
    
    -- Scale threshold with speed: faster speed = larger threshold
    -- Multiply by 3 as safety buffer for jumps/momentum
    local dynamicThreshold = NoclipData.BaseAntiCheatThreshold * speedMultiplier * 3
...
-- FIXED: Detect anti-cheat snapback with speed awareness
local function detectAntiCheatSnapback(newPos, oldPos)
    ...
    -- Get speed-aware threshold
    local threshold = getSpeedAwareThreshold()
    
    -- If distance exceeds threshold, check if it's a snapback
    if distance > threshold then
        ...
        -- Check if position suddenly jumped to a far location inconsistent with history
        if #NoclipData.PositionHistory >= 3 and NoclipData.TeleportCooldown <= 0 then
            ...
            if distanceFromAverage > threshold and not isNearLockButton(newPos) then
                return true
            end
        end
    end
...
local function toggleNoclip(state)
    ...
    if state then
        ...
        Connections.Noclip = RunService.Heartbeat:Connect(function()
            ...
            -- Check for anti-cheat snapback with speed awareness
...
-- SIMPLIFIED: ANTI-VOID SYSTEM (NO MANUAL SAVE)
local function toggleAntiVoid(state)
    ...
    if state then
        ...
        Connections.AntiVoid = RunService.Heartbeat:Connect(function()
            ...
            -- Save position if on solid ground
            ...
            if currentY < AntiVoidData.VoidThreshold then
                ...
                elseif AntiVoidData.LastValidPosition then
                    -- No ground found, use last valid position
        ...
        WindUI:Notify({
            ...
            Content = "Automatic void protection active! No setup needed.",
            Duration = 3,
            Icon = "shield",
        })
    else
