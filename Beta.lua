-- ========================================
-- STEAL A ANIME - WINDUI HUB (FIXED v3)
-- Made by PickleTalk and Mhicel
-- Fixed: Velocity-Based Teleport Detection + Instant Recovery
-- ========================================
...
-- Advanced Noclip Storage (VELOCITY-BASED TELEPORT DETECTION)
local NoclipData = {
    Character = nil,
    Humanoid = nil,
    HRP = nil,
...
-- ========================================
-- VELOCITY-BASED TELEPORT DETECTION (NEW)
-- ========================================
...
-- FIXED: Detect actual teleportation using velocity calculation
local function detectTeleportation(newPos, oldPos, deltaTime)
    -- WHITELIST: Ignore auto-lock teleports
    if NoclipData.IsAutoLocking then
        return false
    end
    
    -- WHITELIST: Ignore if near lock buttons (auto-lock destination)
    ...
    -- Calculate velocity (studs per second)
    local velocity = distance / deltaTime
    
    -- THRESHOLD: Maximum realistic velocity
    -- Even at speed 200 with jump/momentum, realistic max is ~300 studs/second
    -- Teleportation = instant movement = thousands of studs/second
    local maxRealisticVelocity = 300
    
    -- If velocity exceeds threshold, it's a teleport!
    if velocity > maxRealisticVelocity then
        return true
    end
...
local function toggleNoclip(state)
    ...
    if state then
        ...
        -- Main monitoring loop with velocity-based detection
        Connections.Noclip = RunService.Heartbeat:Connect(function()
            ...
            local deltaTime = currentTime - NoclipData.LastFrameTime
            NoclipData.LastFrameTime = currentTime
            
            -- Get current position
            local currentPosition = NoclipData.HRP.Position
            
            -- DETECT TELEPORTATION using velocity calculation
            if detectTeleportation(currentPosition, NoclipData.LastValidPosition, deltaTime) then
                ...
                print("⚡ TELEPORT DETECTED - INSTANT RECOVERY!")
            else
                -- Normal movement - update last valid position
                NoclipData.LastValidPosition = currentPosition
            end
...
local function toggleAntiKick(state)
    ...
    WindUI:Notify({
        ...
        Content = state and "Protected from kicks!" or "Protection disabled.",
        Duration = 3,
...
local AntiVoidToggle = MainTab:Toggle({
    Title = "Anti-Void",
    Desc = "Auto-prevents falling below Y=-50",
    Default = false,
    Callback = function(state)
...
local SpeedToggle = PlayerTab:Toggle({
    Title = "Speed Changer",
    Desc = "Works perfectly with velocity-based noclip!",
    Default = false,
    Callback = function(state)
        toggleSpeed(state)
    end
...
local NoclipToggle = PlayerTab:Toggle({
    Title = "Noclip (VELOCITY-BASED)",
    Desc = "Detects teleports, allows fast speed movement!",
    Default = false,
    Callback = function(state)
        toggleNoclip(state)
    end
...
local PlayerESPToggle = VisualTab:Toggle({
    Title = "Player ESP",
    Desc = "Show outline and name above players",
    Default = false,
    Callback = function(state)
...
local BaseTimeAlertToggle = VisualTab:Toggle({
    ...
    Desc = "Alert when base unlocks in 10s",
    Default = false,
    Callback = function(state)
...
local LowGFXToggle = OptimizationsTab:Toggle({
    Title = "Low GFX Mode",
    Desc = "Saves and restores original GFX!",
    Default = false,
    Callback = function(state)
        toggleLowGFX(state)
    end
...
-- ========================================
-- CREDITS TAB ELEMENTS (LARGER THUMBNAIL)
-- ========================================

local CreditsParagraph = CreditsTab:Paragraph({
    Title = "Steal A Anime Hub | Official",
    Desc = "Made by PickleTalk and Mhicel. Join our discord for latest updates and scripts!",
    Color = "Red",
    Thumbnail = "rbxassetid://74135635728836",
    ThumbnailSize = 140, -- INCREASED from 80 to 140 for better profile picture display
    Buttons = {
        {
            Icon = "users",
            Title = "Discord",
...
local ThemeColorPicker = SettingsTab:Colorpicker({
    ...
    Desc = "Select custom accent color",
