-- ========================================
-- STEAL A ANIME - WINDUI HUB (FIXED)
-- Made by PickleTalk and Mhicel
-- Ultra-Fast Anti-Cheat Protection + God Mode + Anti-Void
-- ========================================
...
-- Advanced Noclip Storage (ANTI-CHEAT SYSTEM)
local NoclipData = {
    Character = nil,
    Humanoid = nil,
    HRP = nil,
...
-- ========================================
-- ULTRA-FAST TELEPORT (FASTER THAN LIGHT - NO DELAYS)
-- ========================================
local function ultraFastTeleport(targetPosition)
    ...
    pcall(function()
        -- INSTANT PHYSICS DISABLE - ALL IN ONE FRAME
        ...
        -- INSTANT TELEPORT - MULTIPLE METHODS SIMULTANEOUSLY (NO DELAYS)
...
-- ========================================
-- ADVANCED ANTI-CHEAT NOCLIP SYSTEM
-- ========================================
...
local function detectAntiCheatSnapback(newPos, oldPos)
    ...
    if #NoclipData.PositionHistory >= 3 and NoclipData.TeleportCooldown <= 0 then
        ...
        if distanceFromAverage > NoclipData.AntiCheatThreshold then
...
-- Refresh noclip character references
    ...
    if state then
        ...
        Connections.Noclip = RunService.Heartbeat:Connect(function()
            ...
            if detectAntiCheatSnapback(NoclipData.CurrentPosition, NoclipData.LastValidPosition) then
                ...
                print("⚡ ANTI-CHEAT BLOCKED - INSTANT TELEPORT!")
            else
...
-- FIXED GOD MODE (FRAME-BY-FRAME RESTORATION)
local function toggleGodMode(state)
    ...
    if state then
        -- Continuous god mode restoration every frame
...
local function saveCurrentSafePosition()
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local currentY = player.Character.HumanoidRootPart.Position.Y
        AntiVoidData.SafeYPosition = currentY
        AntiVoidData.LastSaveTime = tick()
        
        WindUI:Notify({
            Title = "Safe Position Saved",
            Content = string.format("Y Position: %.1f - You won't fall below this!", currentY),
            Duration = 3,
            Icon = "shield",
        })
    end
...
local function toggleAntiVoid(state)
    ...
    if state then
        -- Initialize safe Y position
        if not AntiVoidData.SafeYPosition and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            AntiVoidData.SafeYPosition = player.Character.HumanoidRootPart.Position.Y
        end
        ...
        Connections.AntiVoid = RunService.Heartbeat:Connect(function()
            ...
            if tick() - AntiVoidData.LastSaveTime > AntiVoidData.SaveInterval then
                local currentY = hrp.Position.Y
                if currentY > (AntiVoidData.SafeYPosition or -100) then
                    AntiVoidData.SafeYPosition = currentY
                    AntiVoidData.LastSaveTime = tick()
                end
            end
            
            -- Check if falling below safe position
            if AntiVoidData.SafeYPosition and hrp.Position.Y < AntiVoidData.SafeYPosition - 5 then
                -- INSTANT TELEPORT UP
                local safePos = Vector3.new(hrp.Position.X, AntiVoidData.SafeYPosition, hrp.Position.Z)
                ultraFastTeleport(safePos)
                print("⚠️ ANTI-VOID ACTIVATED - TELEPORTED UP!")
            end
        end)
        
        WindUI:Notify({
            Title = "Anti-Void Enabled",
            Content = "You are now protected from falling into the void!",
            Duration = 3,
            Icon = "shield",
        })
    else
...
local AntiVoidToggle = MainTab:Toggle({
    Title = "Anti-Void Protection (NEW)",
    Desc = "Prevents you from falling into the void - instant teleport up!",
    Default = false,
    Callback = function(state)
...
local SaveSafePositionButton = MainTab:Button({
    Title = "Save Safe Position",
    Desc = "Manually update your safe Y position for anti-void",
    Callback = function()
...
local SpeedToggle = PlayerTab:Toggle({
    Title = "Speed Changer (Advanced)",
    Desc = "Custom speed with professional anti-detect spoofer!",
    Default = false,
    Callback = function(state)
        toggleSpeed(state)
    end
...
local NoclipToggle = PlayerTab:Toggle({
    Title = "Noclip (Anti-Cheat Protected)",
    Desc = "Walk through walls with ultra-fast anti-cheat recovery!",
    Default = false,
    Callback = function(state)
        toggleNoclip(state)
    end
...
-- ========================================
-- CREDITS TAB ELEMENTS (FIXED THUMBNAIL)
-- ========================================
...
WindUI:Popup({
    Title = "Steal A Anime Hub | Official",
    Icon = "sword",
    Content = "NEW UPDATE: Ultra-fast anti-cheat noclip + Fixed God Mode + Anti-Void protection! Enable features from tabs.",
    Buttons = {
        {
            Title = "Close",
