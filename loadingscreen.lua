...
-- Loading bar glow (compatible with original)
local loadingBarGlow = Instance.new("UIStroke")
loadingBarGlow.Color = Color3.fromRGB(180, 230, 255)
...
-- Percentage text with counter animation (compatible name)
...
-- Warning label with icon (compatible with original)
...
-- COMPATIBLE: Enhanced loading bar animation (original function name)
local function animateLoadingBar(callback)
    ...
    local success, err = pcall(function()
        ...
        -- Loading bar glow animation (compatible with original)
...
-- COMPATIBLE: Enhanced entrance animations (original function name)
local function playEntranceAnimations()
    ...
    local success, err = pcall(function()
        ...
        for _, item in pairs(elements) do
            wait(item.delay)
            local transparency = item.element == copyButton and 0 or 0
...
-- COMPATIBLE: Enhanced exit animations (original function name)
local function playExitAnimations(callback)
    ...
    local success, err = pcall(function()
        ...
        TweenService:Create(logoFrame, tweenInfo, {BackgroundTransparency = 1}):Play()
...
-- COMPATIBLE: Dynamic pulse effects (original function name)
...
-- COMPATIBLE: Start the enhanced loading screen (original function name)
...
-- COMPATIBLE: Expose enhanced loading screen functions with original names
return {
