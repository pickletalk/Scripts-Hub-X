...
-- Main Frame (extended height for new button)
local mainFrame = Instance.new("Frame")
mainFrame.Name = "MainFrame"
mainFrame.Size = UDim2.new(0, 250, 0, 170) -- Extended from 120 to 170
...
teleportButton.Position = UDim2.new(0, 15, 0, 90) -- Positioned below float button
teleportButton.BackgroundColor3 = Color3.fromRGB(50, 0, 50)
teleportButton.Text = "👻 ENABLE GHOST MODE 👻"
...
statusLabel.Position = UDim2.new(0, 10, 0, 140) -- Moved down
...
local function makePlayerVisible()
    ...
    for _, part in pairs(player.Character:GetChildren()) do
        ...
        elseif part:IsA("Accessory") then
            local handle = part:FindFirstChild("Handle")
            if handle and originalTransparency[handle] then
                handle.Transparency = originalTransparency[handle]
...
local function enableTeleport()
    ...
    teleportConnection = RunService.Heartbeat:Connect(function()
        ...
        if carpet and carpet:IsA("Part") then
            player.Character.HumanoidRootPart.CFrame = carpet.CFrame + Vector3.new(0, 5, 0)
        end
    end)
    
    teleportButton.BackgroundColor3 = Color3.fromRGB(100, 0, 100)
    teleportButton.Text = "👻 DISABLE GHOST MODE 👻"
...
local function onCharacterAdded(newCharacter)
    ...
    -- Clear transparency table on character respawn
    ...
    -- Re-apply invisibility if teleport was enabled
    if teleportEnabled then
...
local function addHoverEffect(button, hoverColor, originalColor)
    button.MouseEnter:Connect(function()
        ...
        elseif button == teleportButton then
            if teleportEnabled then
                button.BackgroundColor3 = Color3.fromRGB(120, 0, 120)
            else
                button.BackgroundColor3 = Color3.fromRGB(70, 0, 70)
            end
        else
...
addHoverEffect(teleportButton, Color3.fromRGB(70, 0, 70), Color3.fromRGB(50, 0, 50))
...
teleportButton.MouseButton1Click:Connect(function()
    ...
    local clickTween = TweenService:Create(teleportButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {Size = UDim2.new(0, 210, 0, 33)})
    local releaseTween = TweenService:Create(teleportButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {Size = originalSize})
